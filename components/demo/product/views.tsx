  ...
  return (
    <div className="pd-view">
      ...
      <div style={{ marginBottom: 12 }}>
        <h2 style={{ fontSize: 17, fontWeight: 700, margin: 0 }}>Monthly Performance</h2>
        <div className="pd-page-sub">Volume, completions and receipt tracking by month</div>
      </div>

      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
        <span style={{ fontSize: 12, color: "var(--pd-ink-3)", fontWeight: 500 }}>Financial Years:</span>
      ...
      <div className="pd-grid c-2">
        <div className="pd-card">
          ...
          <div style={{ display: "flex", gap: 16, fontSize: 11, marginTop: 6, justifyContent: "center" }}>
            {activeYears.map((y) => (
              <span key={y} style={{ color: colors[y], fontWeight: 700 }}>■ {y}</span>
            ))}
          </div>
        </div>
        <div className="pd-card">
...
export function InsightsView() {
  ...
  const stats: { i: IconKey; l: string; v: string }[] = [
    { i: "CaseMgmt", l: "Total residents", v: "8" },
    { i: "Enquiries", l: "Signed up this month", v: "0" },
    { i: "Pound", l: "Average salary", v: "£53,125" },
    { i: "PiggyBank", l: "Average savings", v: "£17,625" },
  ];
    ...
    <div className="pd-view">
        ...
        {stats.map((s, i) => {
          ...
          return (
            <div key={i} className="pd-card">
              <div className="pd-stat-label"><I style={{ width: 14, height: 14, color: "var(--pd-primary)" }} />{s.l}</div>
              <div className="pd-stat-value">{s.v}</div>
            </div>
          );
        })}
      </div>
      ...
      <div className="pd-card" style={{ padding: "4px 6px" }}>
        <div className="pd-table-wrap"><table className="pd-table">
          <thead>
            <tr>
              <th>Name</th><th>Email</th><th>Category</th><th>Transaction status</th><th>Sign-up date</th><th>Unit</th><th style={{ textAlign: "right" }}>Profile</th>
            </tr>
          </thead>
          <tbody>
            {residents.map((r, i) => (
              <tr key={i}>
                <td className="name">{r.name}</td>
                <td>{r.email}</td>
                <td><span className={`pd-pill ${r.cat.c} tiny`}>{r.cat.l}</span></td>
                ...
                <td style={{ textAlign: "right" }}>
                  ...
                  <span style={{ marginLeft: 8, fontSize: 11, color: "var(--pd-ink-3)", fontWeight: 600 }}>{r.pct}%</span>
                </td>
              </tr>
            ))}
          </tbody>
    ...
    <div className="pd-view">
      <div className="pd-page-head">
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <span className="pd-pill indigo">Pinnacle</span>
          <span className="pd-pill outline">Southern</span>
          <span className="pd-pill outline">CHP</span>
        </div>
      ...
      <div className="pd-grid c-4" style={{ marginBottom: 16 }}>
        <div className="pd-card">
          <div className="pd-stat-label"><Icon.Home style={{ width: 14, height: 14, color: "var(--pd-primary)" }} />Live listings</div>
          <div className="pd-stat-value">42</div>
          <div style={{ fontSize: 11, color: "var(--pd-green)", marginTop: 2 }}>+8 this month</div>
        </div>
        <div className="pd-card">
          <div className="pd-stat-label"><Icon.Mail style={{ width: 14, height: 14, color: "var(--pd-magenta)" }} />Avg viewings / week</div>
          <div className="pd-stat-value">7.4</div>
          <div style={{ fontSize: 11, color: "var(--pd-green)", marginTop: 2 }}>+1.2 vs prev</div>
        </div>
        <div className="pd-card">
          <div className="pd-stat-label"><Icon.Pound style={{ width: 14, height: 14, color: "var(--pd-primary)" }} />Median price</div>
          <div className="pd-stat-value">£295k</div>
          <div style={{ fontSize: 11, color: "var(--pd-ink-3)", marginTop: 2 }}>across 7 developments</div>
        </div>
        <div className="pd-card">
          <div className="pd-stat-label"><Icon.Cal style={{ width: 14, height: 14, color: "var(--pd-primary)" }} />Avg time to offer</div>
          ...
          <tbody>
            {listings.map((r, i) => (
              <tr key={i}>
                ...
                <td>{r.dev}</td>
                <td>{r.beds}</td>
                <td style={{ textAlign: "right", fontWeight: 600, color: "var(--pd-ink)" }}>{r.price}</td>
                <td>{r.share}</td>
        ...
        {stats.map((s, i) => {
          ...
          return (
            <div key={i} className="pd-card" style={s.highlight ? { borderColor: "#f0c97b", background: "#fffaef" } : {}}>
              <div className="pd-stat-label"><I style={{ width: 14, height: 14 }} />{s.l}</div>
          ...
          <tbody>
            {rows.map((r, i) => (
              <tr key={i}>
                ...
                <td style={{ color: r.days > 60 ? "var(--pd-red)" : "var(--pd-ink-2)", fontWeight: 600 }}>{r.days}d</td>
...
export function UnitsView() {
  ...
  const data: Record<typeof period, number[]> = {
    Y: [12, 18, 28, 35, 42, 48, 55, 61, 58, 52],
    Q: [8, 14, 22, 28, 33, 38, 43, 46],
    M: [44, 46, 49, 52, 55, 58, 61],
  };
  const months: Record<typeof period, string[]> = {
    ...
    <div className="pd-view">
      ...
      <div className="pd-grid c-2-1" style={{ marginBottom: 16 }}>
        <div className="pd-card">
          <div className="pd-card-head">
            <div>
              <div className="pd-card-title">New Units</div>
              <div style={{ fontSize: 12, color: "var(--pd-ink-3)", marginTop: 8 }}>Units uploaded</div>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 2 }}>
                <span className="pd-stat-value" style={{ color: "var(--pd-magenta)" }}>61</span>
                <span className="pd-pill pink tiny">↓ 5%</span>
              </div>
            </div>
            <div className="pd-period-switch">
          ...
          <AreaChart data={data[period]} months={months[period]} height={170} />
        </div>

        <div className="pd-card">
          <div className="pd-card-title" style={{ marginBottom: 12 }}>Average Service Charge</div>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <DonutChart
              data={[
                { value: 240, color: "#ff8eb6" },
                { value: 180, color: "#5b3fd6" },
                { value: 140, color: "#7a5ef0" },
                { value: 60, color: "#f0a93b" },
                { value: 45, color: "#b6e3c8" },
              ]}
              centerLabel="£665"
              size={130}
              thickness={18}
            />
            <div className="pd-legend" style={{ fontSize: 11 }}>
              <div className="pd-legend-row"><span className="pd-legend-dot" style={{ background: "#ff8eb6" }} />Building maintenance</div>
              <div className="pd-legend-row"><span className="pd-legend-dot" style={{ background: "#5b3fd6" }} />Utilities & energy</div>
              <div className="pd-legend-row"><span className="pd-legend-dot" style={{ background: "#7a5ef0" }} />Management</div>
              <div className="pd-legend-row"><span className="pd-legend-dot" style={{ background: "#f0a93b" }} />Compliance</div>
              <div className="pd-legend-row"><span className="pd-legend-dot" style={{ background: "#b6e3c8" }} />Reserve</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="pd-card">
        <div className="pd-card-head">
          <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
            ...
            <span className="pd-pill indigo tiny">61 active units</span>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <div className="pd-search-input"><Icon.Search style={{ width: 13, height: 13 }} />Search</div>
            <button className="pd-btn icon"><Icon.Filter style={{ width: 14, height: 14 }} /></button>
          ...
          <tbody>
            {units.map((r, i) => (
              <tr key={i}>
                ...
                <td style={{ textAlign: "right" }}>{r.rent}</td>
                <td>{r.share}</td>
                <td style={{ textAlign: "right", fontWeight: 600, color: "var(--pd-ink)" }}>{r.val}</td>
                <td>{r.occ}</td>
                <td>
  ...
  return (
    <div className="pd-view">
      ...
      <div className="pd-grid c-3" style={{ marginBottom: 16 }}>
        <div className="pd-card">
          <div className="pd-stat-label"><Icon.Devs style={{ width: 14, height: 14, color: "var(--pd-primary)" }} />Active developments</div>
          <div className="pd-stat-value">6</div>
          <div style={{ fontSize: 11, color: "var(--pd-green)", marginTop: 2 }}>+1 in pipeline</div>
        </div>
        <div className="pd-card">
          <div className="pd-stat-label"><Icon.Home style={{ width: 14, height: 14, color: "var(--pd-magenta)" }} />Total units</div>
          <div className="pd-stat-value">130</div>
          <div style={{ fontSize: 11, color: "var(--pd-ink-3)", marginTop: 2 }}>89.8% occupied</div>
        </div>
        <div className="pd-card">
          <div className="pd-stat-label"><Icon.Pound style={{ width: 14, height: 14, color: "var(--pd-primary)" }} />Monthly revenue</div>
        ...
        {devs.map((d, i) => (
          <div key={i} className="pd-card" style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              <div>
                <div style={{ fontSize: 15, fontWeight: 700, color: "var(--pd-ink)" }}>{d.name}</div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8, fontSize: 12 }}>
              <div>
                ...
                <div style={{ fontWeight: 700, fontSize: 16, color: "var(--pd-ink)" }}>{d.units}</div>
              </div>
              <div>
                <div style={{ color: "var(--pd-ink-3)" }}>Occupancy</div>
                <div style={{ fontWeight: 700, fontSize: 16, color: "var(--pd-ink)" }}>{d.occ}%</div>
              </div>
              <div>
                <div style={{ color: "var(--pd-ink-3)" }}>Revenue</div>
                <div style={{ fontWeight: 700, fontSize: 16, color: "var(--pd-ink)" }}>{d.rev}</div>
              </div>
            </div>
            <div>
      ...
      <div className="pd-grid" style={{ gridTemplateColumns: "1fr 280px" }}>
            ...
            <div className="pd-card-head">
              <div>
                ...
                <div style={{ marginTop: 6, fontSize: 13 }}>
                  <span style={{ fontSize: 24, fontWeight: 800, color: "var(--pd-ink)" }}>3</span>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <div className="pd-card" style={{ background: "linear-gradient(170deg, #efe5ff, #f8f0ff)", border: "1px dashed #c5b3f0", textAlign: "center", padding: 20 }}>
            <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.1em", color: "var(--pd-primary)", marginBottom: 12 }}>UPLOAD</div>
            ...
            <div style={{ fontSize: 15, fontWeight: 700, color: "var(--pd-ink)" }}>Add lease documents</div>
